...
- [x] The leading term is the identity
  > The gradient reaches layer 1 undamped no matter what the blocks do. A plain network's backward pass is $\prod_i \partial F_i$; at spectral radius 0.8 and $L=50$ that is $10^{-5}$ and the early layers learn nothing. The residual path is what removes the dependence on depth.
...
- [x] Overfitting means fitting the training set too well
  > Here the deeper model fits it worse, so the problem is optimisation, not generalisation. The deeper network can represent everything the shallower one can by setting 36 layers to the identity. It simply could not find that solution, because learning an exact identity through linear-plus-nonlinear layers is hard and nothing pressures gradient descent towards it.
